...
impl SqliteHandler {
    ...
    fn register_interval_functions(conn: &Connection) -> Result<()> {
        ...
        // extract_from_interval - extract a field from an interval
        conn.create_scalar_function(
    ...
    pub fn register_builtin_functions(conn: &Connection, _functions: Arc<DashMap<String, crate::catalog::FunctionMetadata>>, sessions: Arc<DashMap<u32, SessionContext>>) -> Result<()> {
        ...
        bool_aggregates::register_bool_aggregates(conn)?;
        bool_aggregates::register_bool_statefuncs(conn)?;
        bool_aggregates::register_bitwise_aggregates(conn)?;

        crate::stats_accum::register_stats_accum_functions(conn)?;
        crate::float_special::register_float_special_functions(conn)?;
        crate::float_special::register_float_validation(conn)?;

        // Register JSON aggregate functions
        crate::jsonb::register_json_agg_functions(conn)?;

        // Register interval functions
        Self::register_interval_functions(conn)?;

        // Register special float value functions (NaN, Infinity)
        crate::float_special::register_float_special_functions(conn)?;
        crate::float_special::register_float_validation(conn)?;
